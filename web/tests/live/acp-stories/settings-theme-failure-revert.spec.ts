...
base("PATCH 500 does not silently change dataset.theme or dispatch picker event", async ({ page }, testInfo) => {
  ...
  try {
    ...
    await expect
      .poll(async () => await page.evaluate(() => document.documentElement.dataset.theme ?? ""), { timeout: 5_000 })
      .not.toBe("");
    await page.evaluate(() => {
      (window as unknown as { __pickerFired?: number }).__pickerFired = 0;
    });
    const datasetBefore = await page.evaluate(() => document.documentElement.dataset.theme);
    ...
    const datasetAfter = await page.evaluate(() => document.documentElement.dataset.theme);
