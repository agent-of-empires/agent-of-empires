...
| `session_id_poller_max_threads` | `200` | Ceiling on concurrent session-id pollers per process. Past the ceiling, the overflow's ids refresh on a 5 s to 60 s backoff instead of every status tick. Global only, applied at process start. |
