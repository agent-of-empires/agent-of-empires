...
| `acp.offer_structured_in_new_session` | `true` | Offer the structured view when creating a session, and offer switching an existing terminal session into it. Off hides the choice on both the TUI dialog and the web dashboard's new-session form, so every new session opens as a terminal. Opening an already-structured session, and switching one back to a terminal, are unaffected. |
| `acp.default_new_session_view` | `structured` | Which view the new-session dialog starts on for agents that can back a structured session: `structured` or `terminal`. The dialog's own toggle still wins for that session, so this only sets the opening state. Ignored while `acp.offer_structured_in_new_session` is off. |
