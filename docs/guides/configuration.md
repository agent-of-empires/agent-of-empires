...
| `agent_status_hooks` | `true` | Install status-detection hooks into the agent's config file; see [Adding a New Agent](../development/adding-agents.md#hook-format-reference) for the per-agent file and payload. Config-dir overrides are honored. When disabled, status detection reads the pane alone, but authoritative identity hooks required for native resume remain installed. A hook write is evidence rather than the last word: pane state and terminal title are weighed against it by declared priority. |
...
| `agent_command_override` | `{}` | Per-agent command override replacing the binary entirely (for example, `{ claude = "my-claude-wrapper" }`). Native conversation resume is fail-closed for overrides because AoE cannot prove which process owns the captured id. Use the built-in binary directly when native resume is required. |
