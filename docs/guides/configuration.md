...
Local shell commands run by the core on a status change, for desktop notifications and similar personal automation. They fire while no TUI is open and while one is attached to tmux; the initial snapshot does not replay existing statuses. Off by default, and global/profile only because they run arbitrary commands.
