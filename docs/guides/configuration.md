...
| `acp.offer_structured_in_new_session` | `false` | Offer the structured view when creating a session, and offer switching an existing terminal session into it. The structured view is still maturing, so both are hidden until you turn this on; with it off every new session opens as a terminal on both the TUI dialog and the web dashboard's new-session form. Opening an already-structured session, switching one back to a terminal, and importing an existing agent session are unaffected. |
| `acp.default_new_session_view` | `structured` | Which view the new-session dialog starts on once `acp.offer_structured_in_new_session` is on and the chosen agent can back a structured session: `structured` or `terminal`. The dialog's own toggle still wins for that session, so this only sets the opening state. Ignored while the offer setting is off. |
