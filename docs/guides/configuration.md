...
| `agent_status_hooks` | `true` | Install status-detection hooks into the agent's config file. Codex uses the `[hooks]` table in `~/.codex/config.toml`; other JSON-based agents use their settings JSON. Config-dir overrides are honored: if `CLAUDE_CONFIG_DIR` (Claude) or `CURSOR_CONFIG_DIR` (Cursor) is set in the session's profile environment or in AoE's own environment, hooks are written to that directory instead of `~/.claude` / `~/.cursor`. When disabled, status detection falls back to tmux pane content parsing. Codex is hook-first, but known hook gaps are reconciled from pane content. |
