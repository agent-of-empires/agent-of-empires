...
| `session_id_poller_max_threads` | `50` | Ceiling on concurrent session-id pollers per process. Past the ceiling, idle pollers hand their thread to waiting sessions, so ids refresh about once a minute instead of every status tick. Global only, applied at process start. |
