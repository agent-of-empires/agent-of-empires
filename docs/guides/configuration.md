...
On secrecy, the terminal view is better than the static `environment` list but not airtight: a static entry becomes a shell-assignment prefix on the pane command and is therefore visible in `ps` for the pane's whole life, whereas a minted value only rides the short-lived `tmux` client invocation. That invocation's argv is still momentarily visible. For a value that must never touch argv at all, use a sandboxed session and `before_start`, which passes values to `docker` through the process environment.
