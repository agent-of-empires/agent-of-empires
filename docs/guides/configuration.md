...
`work-claude` is now structured view-capable through `claude-agent-acp`, and the existing terminal session's **Switch to structured view** action becomes available. The wrapper runs through the base adapter (inheriting its version gate and env allowlist), so it renders exactly like the base agent. Its profile/oauth overrides reach the structured worker through the session's `extra_env` / [Host Environment](#host-environment) (for example `environment = ["CLAUDE_CONFIG_DIR=/Users/me/.claude-work"]`) or `session.inherit_host_environment`, the same as any other agent. This only works when the base has a built-in ACP adapter (`claude`, `codex`, `opencode`, `gemini`, `vibe`, `pi`, `omp`, `kimi`, `prime-agent`); a wrapper mapped to a terminal-only base (e.g. `cursor`) stays tmux-only. An explicit `agent_acp_cmd` still wins if you set both.
