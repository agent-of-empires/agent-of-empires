...
| `agent_command_override` | `{}` | Per-agent command replacing the binary, for example `{ claude = "claude --model opus" }`. Managed resume/fork validates the actual native command and store; opaque wrappers require an explicit execution contract. See [session resume](session-resume.md#execution-identity-and-wrappers). |
...
| `agent_detect_as` | `{}` | Built-in status detection and ACP adapter inheritance for a custom agent. This is not authority for terminal resume or fork; see [execution identity](session-resume.md#execution-identity-and-wrappers). |
...
- **`agent_detect_as`** (optional): Reuses built-in status detection and ACP adapter inheritance. It never proves which native CLI owns a terminal conversation.
- **`agent_execution_as`** (required for managed opaque wrappers): Declares the native agent actually invoked, together with its `agent_config_dir` store contract. See [native resume](session-resume.md#execution-identity-and-wrappers).
...
Custom agents remain selectable even for remote or unmanaged commands. These maps are editable in configuration files or the TUI settings screen. Profile maps replace global maps, so redeclare entries you want to retain. Repository configuration may override status detection, but cannot declare trusted execution identity. The web wizard selects configured agents without editing command strings.
