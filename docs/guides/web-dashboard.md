...
- **Step-up elevation**: a "Confirm passphrase" prompt appears on writes that can plant code for, or widen what is exposed to, the next session spawn. That covers the `sandbox` and `worktree` sections plus individual fields that carry the same risk, currently `acp.restrict_agents`, `skills.auto_propagate`, `session.smart_rename_model`, and `session.inherit_host_environment`; the gate is per field, so the rest of a section saves without it. Confirmation lasts 15 minutes. User-preference writes (theme, sound, notifications, etc.) save without it. Localhost browsers skip the prompt, because the same-host caller already passes the filesystem trust boundary. `--auth=passphrase --behind-proxy` is the exception: proxied traffic reaches the daemon over a loopback socket, so an upstream that does not forward the client IP would hand a remote visitor that bypass. Under that combination the prompt applies to loopback callers too. See [Settings & profiles](web/settings.md#step-up-elevation).
