...
The terminal owns the full viewport on phones. A floating keyboard button toggles the soft keyboard, and a terminal toolbar exposes a `Ctrl` modifier toggle. The terminal stays mounted (hidden, not unmounted) while you switch panes so xterm.js keeps its geometry, scrollback, and focus.
