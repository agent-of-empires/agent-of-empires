...
- **Hermes:** an explicitly declared configuration root and its `state.db`, with local terminal execution. The profile is pinned without following the sticky active profile. Dotenv credentials are allowed; dotenv routing overrides, managed configuration, container delegation and enabled external secret sources are refused. This does not enable host capture.
