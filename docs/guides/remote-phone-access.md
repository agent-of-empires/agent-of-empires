...
- **Sessions survive a daemon restart.** Login sessions are persisted to an owner-only file in the app dir, so restarting `aoe serve` (a config edit, `aoe update`, or a crash) no longer logs every device out. The persisted store is dropped if you change the passphrase, and you can disable persistence with `auth.persist_sessions = false` to force re-authentication on every restart. Step-up elevation is intentionally NOT persisted: a high-risk action re-prompts for the passphrase after a restart even though the session itself survives.
