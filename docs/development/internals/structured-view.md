...
- `/compact` (#2898): detected from the adapter's `Compacting...` / `Compacting completed.` text chunks, since it surfaces no typed ACP signal. The start marker latches the floor so a large compaction is never cut short, and the completion marker drops it. The same markers now also publish `ConversationCompactionStarted` / `ConversationCompacted` so both clients know the phase and stop reading its silence as a wedge (#3219); the detector is not profile-gated, so daemon and clients cannot disagree about whether a compaction is running.
