...
- **Calls outstanding at detach.** Reverse and forward calls are attachment-scoped. On disconnect, reverse calls are cancelled toward the agent, forward correlations and queued replies are discarded, and late agent answers are ignored. They are never replayed into a daemon that lacks the correlation. `terminal/release` is globally idempotent cleanup, including after daemon restart, so it needs no tombstone cache.
- **Mid-turn reattach.** A compatible new daemon resumes the runner-owned ACP session directly. `PromptCompleted` survives detach and reports the real outcome; rate-limit errors produce `RateLimit` before `Stopped { reason: "rate_limited" }`. A 30-second resume-idle watchdog remains a fallback. Incompatible runner generations follow the explicit interruption path above.
