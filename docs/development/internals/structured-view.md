...
Notifications and turn completion share an ordered runner-control channel. That wire order is not an application-event completion barrier: asynchronous SDK notification dispatch can lag terminal or handshake bookkeeping.

`runner_version` is an attachment-compatibility generation, separate from liveness and `build_version`. Generation 4 requires the runner to announce authoritative native identity before session callbacks; the control wire version remains 3. A live incompatible runner is reaped before replacement. If its event log shows an in-flight turn, the daemon records `Stopped { reason: "runner_protocol_upgraded" }`; it cannot truthfully drain a protocol it cannot attach to.
...
- **Calls outstanding at detach.** Reverse and forward calls are attachment-scoped. On disconnect, reverse calls are cancelled toward the agent and queued replies are discarded. A pending conversation reset retains runner-side bookkeeping until its result commits the new session identity; it does not replay the old caller's reply. Other late forward answers are ignored. Within the admitted native session, `terminal/release` is idempotent cleanup, including after daemon restart, so it needs no tombstone cache.
- **Native-session admission.** Only lifecycle results select native identity, never notification or callback metadata. Unrelated updates are ignored and unrelated session callbacks are rejected before transcript, permission, filesystem, or terminal effects. While identity is pending, retained notifications are bounded to the runner's detached control queue (the same frame count and encoded-byte ceiling it buffers), so a reattach admits the whole flushed backlog; overflow beyond that fails the connection rather than publishing a partial replay. Matching new/fork/reset updates follow the assignment boundary, and successful reset replay follows clear/context-reset/assignment. Unknown pre-response callback identities are rejected; callbacks matching a received lifecycle result wait for commitment. Established updates incur no serialization for this accounting. Existing stored history is neither rewritten nor deleted by this guard.
- **Callback parameter parity.** On cached reattach, session-scoped filesystem, terminal, and permission callbacks wait for native identity announcement with either named or positional parameters. Request-scoped elicitation remains independent of session authority. The runner preserves the raw callback parameters; the SDK still validates their schema and the daemon still enforces identity.
- **Replay byte accounting.** Pending replay uses the runner's 4096-frame / 128 MiB budget and charges each original control frame, including its four-byte prefix. Typed notification serialization can expand native JSON and is not a valid substitute. The private SDK transport carries reader-measured costs without adding nesting, preserves object and positional-array parameters, and removes its accounting marker before typed decoding. Native fields cannot override that cost; direct stdio retains bounded typed-payload accounting.
