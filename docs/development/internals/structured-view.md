...
- **Mid-turn reattach.** `ResumeSession` obtains the runner's committed session identity, waiting for a reset sent by the previous daemon to settle. It never sends ACP `session/new` or `session/load`. `PromptCompleted` survives detach and reports the real outcome; rate-limit errors produce `RateLimit` before `Stopped { reason: "rate_limited" }`. A 30-second resume-idle watchdog remains a fallback. Incompatible runner generations follow the explicit interruption path above.
