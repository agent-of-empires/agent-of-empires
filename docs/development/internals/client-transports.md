...
`web/src/lib/types.ts` re-exports the generated types under the names the
dashboard already used, and holds only what the wire cannot describe: the view
models the client derives from the rows it fetched (sidebar groups, the diff
hunks `diffPair` produces) and `ProfileSettingsResponse`, whose overrides are a
flattened open map on the Rust side and so have no shape to generate.
