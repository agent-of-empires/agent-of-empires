...
- **Structured / ACP sessions**: `apply_acp_overlay_inplace` in `src/server/mod.rs` is the sole authority. `decide_passive_transition` returns `patch: None` for `is_structured()` rows, so the passive-status writers deliberately skip them. `last_accessed_at` is user-gesture-only here too since #3465: `apply_status_intent` used to stamp it on every worker-event transition, which relayed through `DaemonStatusPoller` into TUI memory and reached disk via `merge_from_tui`'s ungated monotone max, so it no longer writes the field at all. The gestures that do write it are prompting (`touch_on_prompt_and_wake_if_sunk` in `src/server/api/acp.rs`, which lifts sink state only when the prompt actually wakes a sunk row) and queueing a follow-up (`SessionService::enqueue_prompt`, a monotone recency advance that never lifts a sink, since a client only queues behind a live turn). Post-daemon-restart the row's `idle_entered_at` resets to the last durable value (either creation-time, or the last explicit user action); ACP event handlers re-emit as they observe new state.
