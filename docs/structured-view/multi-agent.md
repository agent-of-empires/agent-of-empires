...
AoE's structured view (the web dashboard's structured-rendering view) speaks
...
- **TodoWrite / Skill / ExitPlanMode / ScheduleWakeup** are claude-only
  tools today, so the cards stay quiet on other agents. The structured view
...
- **`/clear`** is detected server-side by matching the user's prompt
  ...
  structured view doesn't treat it as a clear. The composer's `/` palette also
...
- **Subagent indentation** requires the adapter to emit a
  ...
  been verified, so the structured view doesn't render the indent until the
...
- **Server (Rust)**: `src/acp/agent_profiles.rs`. Carries
...
The structured view then renders the generic tool card, which is the right
...
- **CLI:** `aoe acp switch-agent <session> <target>`, where `<target>` is a registry key from `aoe acp agents`. Add `--model <name>` to override the model.
...
- Codex / opencode / gemini / cursor structured view support has been built from adapter
  ...
  observed on the structured view side and we'll update the profile.
...
2. Check the agent's profile in `web/src/lib/agentProfiles.ts`. The
   ...
   directly.
3. If the structured view fired a claude-only card (TodoCard / SkillCard) on a
