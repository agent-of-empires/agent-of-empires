...
`gemini --acp`, `cursor-agent acp`, `vibe-acp`). Custom sandbox images must
include the same adapters or the `docker exec` invocation fails with exit
status 127 and the ACP handshake times out after 30s.

Known limitations:

- `fs/*` path translation only covers the workspace mount(s) the container was
  built with. Agent-config mounts, bind-mounted credentials, and user-configured
  `extra_volumes` are not in the path map. The inside-roots check rejects those
  paths, so the safety property holds; the failure mode is a generic "outside
  session roots" error.
...
aoe requires `claude-agent-acp` v0.39.0 or newer. Install the official adapter:
...
an internal mirror, ship 0.39.0 from the mirror or run the `@latest` install
