...
Some agents put a **question** in the permission options rather than an allow/deny vocabulary: pi's `ask_user_question`, for instance, sends one `allow_once` option per answer. The daemon spots this (two or more options share a kind, so Allow / Always / Deny cannot say which one you meant) and the card renders the agent's own labels instead, one button each; picking one sends back that option. **Dismiss** answers nothing. In the TUI the same card offers `a` to open the answer picker instead of allow-once.
...
The rich form is web-first. The native TUI answers the common case natively: when every required question is single-select (the AskUserQuestion shape), `a` from the transcript pane opens a picker per question and submits the choices; the optional free-text "Other" box is TUI-omitted. Forms with required text / multi-select / number fields still point at the web dashboard, and `s` / `c` skip or cancel from the transcript pane so a TUI-only session never stalls on a question. Once answered (from any surface) the TUI transcript records the chosen answer too.
