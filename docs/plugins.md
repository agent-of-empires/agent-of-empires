...
Updating from the CLI, TUI, or dashboard restarts the plugin's worker in a
running `aoe serve` daemon. If the CLI or TUI cannot reach that daemon it warns,
and the worker keeps the previous build until the daemon restarts.
