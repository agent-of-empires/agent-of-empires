...
- Either way, switch an existing session from the session view at any time: the web sidebar's switch action, or the TUI's right-click context menu (**Switch to structured** / **Switch to terminal**, which needs a running `aoe serve` daemon). The agent restarts in a fresh pane; the worktree, open files, and commits are preserved, but the in-memory conversation for that session resets.
