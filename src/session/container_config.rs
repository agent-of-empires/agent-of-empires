...
const AGENT_CONFIG_MOUNTS: &[AgentConfigMount] = &[
    ...
    AgentConfigMount {
        ...
        // additional approvals beyond the HMP-managed ones.
...
pub(crate) fn compute_volume_paths(
    ...
    // which would cause hmp to mount that ancestor -- potentially the user's entire $HOME --
    // into the container.
    //
...
fn named_volume_for(session_id: &str, container_path: &str) -> String {
    ...
    format!("hmp-vi-{}-{}-{}", session_id, slug, hash12)
}
        ...
        if hooks_enabled {
            ...
            if hermes_hooks || kiro_hooks || agent.hook_config.is_some() {
                let hook_dir = crate::hooks::hook_status_dir(instance_id).context(
                    "refusing to mount hook directory: HMP_INSTANCE_ID failed validation",
                )?;
            ...
            if hermes_hooks {
                ...
                let config_file = sandbox_dir.join("agents").join("hmp-hooks.json");
...
mod tests {
    ...
    fn test_compute_volume_paths_non_bare_worktree_nested_layout() {
        ...
        // HMP's create_worktree converts .git to relative paths via
    ...
    fn test_build_container_config_includes_repo_sandbox_settings() {
        ...
        let config_dir = project_dir.path().join(".hmp");
...
"#,
    ...
    /// Regression: when project_path is a sibling worktree, `.hmp/config.toml`
    ...
    fn test_build_container_config_sibling_worktree_loads_main_repo_extra_volumes() {
        ...
        // Main repo with repo config under .hmp/
        ...
        let config_dir = main_repo.join(".hmp");
        fs::create_dir_all(&config_dir).unwrap();
        fs::write(
            config_dir.join("config.toml"),
            r#"
...
"#,
    ...
    fn test_build_container_config_installs_codex_hooks_files() {
        ...
        assert!(codex_config.contains("hmp-hooks"));
...
"#,
        ...
        assert!(config_text.contains("hmp-hooks"));
        crate::hooks::cleanup_hook_status_dir(instance_id);
    }
...
"#,
    ...
    fn test_volume_ignores_applied_to_parent_repo_mount() {
        ...
        let config_dir = repo_path.join(".hmp");
        fs::create_dir_all(&config_dir).unwrap();
        fs::write(
            config_dir.join("config.toml"),
            r#"
...
"#,
    ...
    fn test_volume_ignores_applied_to_bare_repo_worktree() {
        ...
        // resolve_config_with_repo loads it from there (find_main_repo) even
        // when the session targets a sibling worktree.
        let config_dir = main_repo_path.join(".hmp");
        fs::create_dir_all(&config_dir).unwrap();
        fs::write(
            config_dir.join("config.toml"),
            r#"
...
"#,
    ...
    fn test_named_volume_for_starts_with_prefix() {
        ...
        assert!(name.starts_with("hmp-vi-sess-xyz-"));
    }
    ...
    fn test_named_volume_for_prefix_does_not_match_longer_session_id() {
        ...
        // The cleanup prefix is "hmp-vi-{session_id}-" (trailing dash), so a volume
        // named "hmp-vi-sess10-..." must NOT start with "hmp-vi-sess1-".
        let vol_sess10 = named_volume_for("sess10", "/workspace/node_modules");
        let prefix_sess1 = format!("hmp-vi-{}-", "sess1");
        assert!(
