...
impl Instance {
    ...
    fn ensure_before_start_env(&mut self, force: bool) -> Result<()> {
        ...
        // per-session value (e.g. `$TEST_VAR`) to scope what it mints. Resolved
        // without repo overrides so an untrusted repo can't influence the host
        // hook's environment.
        let session_env = self
            .sandbox_info
            .as_ref()
            .map(|sb| super::environment::session_host_env_pairs(&self.source_profile, sb))
            .unwrap_or_default();
