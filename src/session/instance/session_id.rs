...
impl Instance {
    ...
    fn acquire_session_id_with(
        ...
        if let Some(stored) = self.agent_session_id.clone() {
            ...
            // Which observation is admissible depends on the directory.
            // Claude's non-hook capture is an mtime scan of
            // `<config>/projects/<encoded-cwd>/`: with one session per cwd
            // that scan is sound, and it is what re-targets a stored id after
            ...
            // swapping conversations between panes and leaving one
            // conversation open in two. Where the directory is shared, only
            // the per-instance hook sidecar may retarget an id we already
            // hold.
    ...
    fn claude_project_dir_is_shared(&self) -> bool {
        crate::session::capture::claude_project_dir_is_shared(self)
    }
    ...
    /// Reads without the freshness window: age is not evidence that the
    /// sidecar names the wrong conversation, and on a mass recovery every
    /// sidecar is stale at once.
...
mod tests {
    ...
    mod verify_on_resume {
        use super::super::super::test_helpers::seed_disk_for_sidecar_test;
        use super::*;
        ...
        fn mtime_fallback_declines_when_project_dir_is_shared() {
            ...
            peer_inst.agent_session_id = Some("22222222-2222-4222-8222-222222222222".to_string());
            seed_disk_for_sidecar_test(profile, &peer_inst);
