    ...
    pub(super) fn retroactive_capture_exclusion_set(&self) -> HashSet<String> {
        ...
        crate::session::capture::compose_exclusion_with_persisted_peers(
            ...
            capture_agent == "claude"
                || (matches!(capture_agent, "opencode" | "codex" | "kimi") && !self.is_sandboxed()),
            &self.effective_profile(),
            &self.retroactive_capture_excludes,
        )
    }

    fn opencode_shared_store_peers(&self) -> crate::session::capture::OpenCodePeerState {
        crate::session::capture::opencode_shared_store_peers(&self.id, &self.project_path)
    }
...
mod tests {
    ...
    const OC_SELF: &str = "019342ab-1234-7def-8901-oc0000000001";
    const OC_PEER: &str = "019342ab-1234-7def-8901-oc0000000002";
    const OC_FRESH: &str = "019342ab-1234-7def-8901-oc0000000003";

    fn oc_guard(
        temp: &std::path::Path,
        db: &std::path::Path,
        extra: &[(&'static str, std::path::PathBuf)],
    ) -> EnvGuard {
        let mut pairs = vec![
    ...
    fn oc_seed_store(db: &std::path::Path, rows: &[(&str, &str, i64)]) {
        let conn = rusqlite::Connection::open(db).unwrap();
        conn.execute_batch("CREATE TABLE session (id TEXT PRIMARY KEY, directory TEXT NOT NULL, time_updated INTEGER NOT NULL);").unwrap();
    ...
    fn oc_seed_instance(profile: &str, inst: &Instance) {
    ...
    fn oc_write_config(profile: &str, body: &str) {
        let dir = crate::session::get_profile_dir_path(profile).unwrap();
        std::fs::create_dir_all(&dir).unwrap();
        std::fs::write(dir.join("config.toml"), body).unwrap();
    }

    fn oc_me(profile: &str, cwd: &std::path::Path) -> Instance {
        let mut inst = Instance::new("me", cwd.to_str().unwrap());
        inst.source_profile = profile.to_string();
        inst.tool = "opencode".to_string();
        inst.agent_session_id = Some(OC_SELF.to_string());
        inst
    }

    #[test]
    #[serial_test::serial]
    fn opencode_shared_store_guard_is_mutation_sensitive() {
        struct Peer {
            profile: &'static str,
        ...
        let cases = [
            ...
            (
                ...
                None,
                Peer {
                    profile: "oc-unrelated",
                    config: Some("environment = [\"RUST_LOG=debug\"]\n"),
                    parked: false,
                },
            ),
            (
                "own-redirect",
                Some("environment = [\"OPENCODE_DB=/elsewhere/other.db\"]\n"),
                Peer {
                    profile: "oc-default-peer",
                    config: None,
                    parked: false,
                },
            ),
            (
                ...
                Peer {
                    profile: "oc-var-peer",
                    config: Some("environment = [\"OPENCODE_DB=$OC_TEST_DB\"]\n"),
                    parked: false,
                },
            ),
            (
        ...
        for (name, own_config, peer) in cases {
            ...
            let extra = if peer.profile == "oc-var-peer" {
                vec![("OC_TEST_DB", db.clone())]
            } else {
                vec![]
            };
            let _env = oc_guard(temp.path(), &db, &extra);
            oc_seed_store(
                &db,
                &[
                    (OC_PEER, cwd.to_str().unwrap(), 1_000),
                    (OC_FRESH, cwd.to_str().unwrap(), 2_000),
                ],
            );
            if let Some(config) = own_config {
                oc_write_config("oc-self", config);
            }
            let me = oc_me("oc-self", &cwd);
            oc_seed_instance("oc-self", &me);
            if let Some(config) = peer.config {
                oc_write_config(peer.profile, config);
            }
            let mut other = Instance::new("peer", cwd.to_str().unwrap());
            other.source_profile = peer.profile.to_string();
            if peer.parked {
                other.tool = "claude".to_string();
                other.prior_tool_session_ids.insert(
                    "opencode".to_string(),
                    PriorToolSession {
                        agent_session_id: Some(OC_FRESH.to_string()),
                        acp_session_id: None,
                    },
                );
            } else {
                other.tool = "opencode".to_string();
                other.status = Status::Stopped;
                other.agent_session_id = Some(OC_PEER.to_string());
            }
            oc_seed_instance(peer.profile, &other);
            assert_eq!(me.try_retroactive_capture(), None, "case {name}");
        }
    }

    #[test]
    #[serial_test::serial]
    fn opencode_retroactive_capture_still_runs_without_peer() {
        let temp = tempfile::tempdir().unwrap();
        let cwd = temp.path().join("cwd");
        std::fs::create_dir_all(&cwd).unwrap();
        let db = temp.path().join("opencode.db");
        let _env = oc_guard(temp.path(), &db, &[]);
        oc_seed_store(&db, &[(OC_SELF, cwd.to_str().unwrap(), 1_000)]);
        let me = oc_me("oc-solo", &cwd);
        oc_seed_instance("oc-solo", &me);
        assert_eq!(me.try_retroactive_capture(), Some(OC_SELF.to_string()));
    }

    #[test]
    #[serial_test::serial]
    fn opencode_poller_revalidates_idless_peer_after_observation() {
        let temp = tempfile::tempdir().unwrap();
        let cwd = temp.path().join("cwd");
        std::fs::create_dir_all(&cwd).unwrap();
        let db = temp.path().join("opencode.db");
        ...
        let me = oc_me("oc-self", &cwd);
        oc_seed_instance("oc-self", &me);
        let mut peer = Instance::new("peer", cwd.to_str().unwrap());
        peer.source_profile = "oc-idless".to_string();
        peer.tool = "opencode".to_string();
        peer.status = Status::Stopped;
        oc_seed_instance("oc-idless", &peer);

        let state = me.opencode_shared_store_peers();
        assert!(state.shared && state.ambiguous && state.owner_sids.is_empty());
        ...
        conn.execute(
            "INSERT INTO session (id, directory, time_updated) VALUES (?1, ?2, ?3)",
            rusqlite::params![OC_PEER, cwd.to_str().unwrap(), 1_000],
        )
        .unwrap();
        assert_eq!(
            try_capture_opencode_session_id(cwd.to_str().unwrap(), &Default::default(), Some(0.0))
                .unwrap(),
            OC_PEER,
        );
        assert_eq!(poll(), None);
