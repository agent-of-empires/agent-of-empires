...
mod tests {
    ...
    #[test]
    fn install_hook_errors_classify_read_only_filesystems() {
        {
            let cases = [
                (
            ...
            #[cfg(unix)]
            {
                ...
                let reported =
                    first_read_only_report(&target).expect("first report names the target");
    ...
    #[test]
    #[serial_test::serial]
    fn host_hook_path_disclosure_gate_skips_refuses_and_requires_ack() {
        {
            let temp = tempfile::tempdir().unwrap();
            let _app = crate::session::test_support::isolate_app_dir_at(temp.path());
