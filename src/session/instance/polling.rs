...
impl Instance {
    ...
    pub(super) fn maybe_start_poller_since(&mut self, omp_metadata: Option<OmpCaptureMetadata>) {
        ...
        // profile's `sessions.json`, which is far too expensive for a poll
        // loop. A peer created on this cwd after the poller starts therefore
        // does not flip an already-running "sole writer" verdict; what covers
        // that peer is the per-tick exclusion set, which drops its sid as
        // soon as it publishes one to tmux env.
        let project_dir_is_shared = crate::session::capture::claude_project_dir_is_shared(self);
        if tool == "omp" {
