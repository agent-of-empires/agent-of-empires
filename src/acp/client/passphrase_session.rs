...
//! secret and the session cookie are cached beside `serve.token` under the
//! app dir (owner-only, see `local_session_dir`) so a same-host CLI
//! invocation reuses one long-lived login rather than minting a fresh
//! device session on every process.
...
/// In-memory cache for one `HttpClient`, backed by an on-disk cache for a
/// loopback local daemon (see `DaemonEndpoint::local_session_dir`).
...
impl PassphraseSessionCache {
    ...
    pub(super) fn invalidate(&self, endpoint: &DaemonEndpoint) {
        *self.0.write().unwrap_or_else(|e| e.into_inner()) = None;
        if let Some(dir) = endpoint.local_session_dir() {
...
fn device_binding_secret(endpoint: &DaemonEndpoint) -> String {
    let Some(dir) = endpoint.local_session_dir() else {
...
fn load_persisted(endpoint: &DaemonEndpoint) -> Option<PassphraseSession> {
    let dir = endpoint.local_session_dir()?;
...
mod tests {
    ...
    #[test]
    fn device_binding_secret_ephemeral_without_local_session_dir() {
    ...
    fn invalidate_clears_memory_and_deletes_persisted_cookie() {
        // Regression: `invalidate` used to drop only the in-memory copy, so
        // the very next `get` reloaded the same rejected cookie straight
        // back off disk and the retry-after-401 flow resent it unchanged.
        let dir = tempfile::tempdir().unwrap();
        let endpoint = local_endpoint(dir.path());
        let secret = generate_binding_secret();
        write_owner_only(&dir.path().join(DEVICE_BINDING_FILENAME), &secret);
