...
impl HomeView {
    ...
    pub fn finalize_live_send_resize(&mut self) {
        ...
        // Register the dedup only when the authoritative resize succeeds. If
        // the pane died between state install and this call, the worker path
        // retains ownership of bounded retries without a synchronous probe.
