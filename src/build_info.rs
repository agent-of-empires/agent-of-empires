...
//! `HMP_BUILD_VERSION` rustc-env. It pairs the package version with a git
...
//! workers left running on an older binary after `hmp update`. See #1754.

/// Build identity of this binary. Stamped by `build.rs`; falls back to the
/// package version when `HMP_BUILD_VERSION` was not emitted.
pub const BUILD_VERSION: &str = match option_env!("HMP_BUILD_VERSION") {
    Some(v) => v,
