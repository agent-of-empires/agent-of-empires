...
use crate::daemon::{
    CleanupDefaults, ListSessionsQuery, PlanSummary, SessionResponse, SessionsEnvelope,
...
mod tests {
    ...
    async fn list_sessions_state_filter() {
        ...
        let wire = serde_json::to_vec(&all.0).unwrap();
        let decoded: crate::daemon::SessionsEnvelope = serde_json::from_slice(&wire).unwrap();
        assert_eq!(ids(&decoded).len(), 3);
        assert!(decoded.sessions.iter().all(|session| !session.unread));
        assert!(decoded
            .sessions
            .iter()
            .all(|session| session.warnings.is_empty()));
        assert!(decoded
            .sessions
            .iter()
            .all(|session| !session.monitor_active));
