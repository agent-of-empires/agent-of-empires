...
mod tests {
    ...
    #[test]
    #[serial_test::serial]
    fn sync_failures_keep_originals_retryable() {
        // receipt namespace sync failure preserves original until retry
        {
        ...
        // stage parent sync failure keeps planned original and retries
        {
            let temporary = tempfile::tempdir().unwrap();
            let _environment = crate::session::test_support::isolate_app_dir_at(temporary.path());
            let home = dirs::home_dir().unwrap();
            ...
            let (path, mut receipt) = checked_receipt(&app, &row, "codex", &roots).unwrap();
            let _failure = SyncFailureGuard;
            crate::session::anchored_fs::FAIL_SYNC_ONCE
                .set(Some(root.parent().unwrap().to_path_buf()));
            assert!(
        ...
        // unavailable home preserves pending content for later reconciliation
        {
            let temporary = tempfile::tempdir().unwrap();
            let _environment = crate::session::test_support::isolate_app_dir_at(temporary.path());
            let app = crate::session::get_app_dir().unwrap();
        ...
        // recovery parent sync failure keeps original and retryable receipt
        {
            let temporary = tempfile::tempdir().unwrap();
            let _environment = crate::session::test_support::isolate_app_dir_at(temporary.path());
            let home = dirs::home_dir().unwrap();
    ...
    #[test]
    fn mount_proof_and_stage_retry_cases() {
        // nested stage parent sync failure retries existing components
        {
            let temporary = tempfile::tempdir().unwrap();
        ...
        // ordinary volume only blocks a mount proof when destinations overlap
        {
    ...
    #[test]
    #[serial_test::serial]
    fn retired_context_is_recorded_and_scoped() {
        ...
        // transaction's recovery paths.
        {
            ...
            preserved["resume_intent"] =
                serde_json::json!({"kind":"Use","value":"old-native-context"});
    ...
    #[test]
    #[serial_test::serial]
    fn per_agent_native_content_is_isolated() {
        // codex private sessions survive isolation without host history
        {
            let temporary = tempfile::tempdir().unwrap();
            let _environment = crate::session::test_support::isolate_app_dir_at(temporary.path());
            let home = dirs::home_dir().unwrap();
        ...
        // opencode carries only its native database not a host copyable backup
        {
            let temporary = tempfile::tempdir().unwrap();
            let _environment = crate::session::test_support::isolate_app_dir_at(temporary.path());
            let home = dirs::home_dir().unwrap();
            ...
            host_db
                .execute_batch(
                    "CREATE TABLE history(value TEXT); INSERT INTO history VALUES ('host');",
                )
                .unwrap();
            drop(host_db);
            let mut row = serde_json::to_value(&instance).unwrap();
        ...
        // retired gemini kimi and prime keep only their own native conversation
        {
        ...
        // hermes migration seeds single link controls into an empty private store
        {
            use std::os::unix::fs::MetadataExt;
    ...
    #[test]
    #[serial_test::serial]
    fn conversation_retention_across_links_and_intents() {
        // linked conversation directories do not block content migration
        {
            ...
            for (tool, role) in [
                ...
                for (internal, index_loop) in [(false, false), (true, false), (false, true)] {
                    ...
                    fs::create_dir_all(&project).unwrap();
                    let mut instance =
                        crate::session::Instance::new(tool, project.to_str().unwrap());
                    ...
                    if index_loop {
                        ...
                        symlink("session_index.jsonl", root.path.join("session_index.jsonl"))
                            .unwrap();
                    } else {
        ...
        // a looped carried root does not stop migration of other rows
        {
            use std::os::unix::fs::symlink;
        ...
        // ambiguous or deleted native conversation is retained only in recovery
        {
            use crate::acp::supervisor::SandboxContinuation;
        ...
        // acp continuation intent controls only the requested history lane
        {
            use crate::acp::supervisor::SandboxContinuation;
    ...
    #[test]
    #[serial_test::serial]
    fn staging_and_publication_recover_after_interruption() {
        ...
        // a staged receipt is rebuilt from the lastopped content
        {
            let temporary = tempfile::tempdir().unwrap();
            let _environment = crate::session::test_support::isolate_app_dir_at(temporary.path());
            let home = dirs::home_dir().unwrap();
            ...
            let (receipt_path, mut receipt) =
                checked_receipt(&app, &row, "claude", &roots).unwrap();
        ...
        // a fresh stage renamed into place survives deferral and retry
        {
            let temporary = tempfile::tempdir().unwrap();
            let _environment = crate::session::test_support::isolate_app_dir_at(temporary.path());
            let home = dirs::home_dir().unwrap();
        ...
        // fresh publication recovers before certificate commit
        {
            let temporary = tempfile::tempdir().unwrap();
            let _environment = crate::session::test_support::isolate_app_dir_at(temporary.path());
            let home = dirs::home_dir().unwrap();
            ...
            let config = crate::session::Config::default();
            let instance =
                crate::session::Instance::new("fresh", temporary.path().to_str().unwrap());
            let roots = container_config::sandbox_content_roots(
                "claude",
                None,
                &config.session,
        ...
        // stopped original is preserved before fresh content is certified
        {
            let temporary = tempfile::tempdir().unwrap();
            let _environment = crate::session::test_support::isolate_app_dir_at(temporary.path());
            let home = dirs::home_dir().unwrap();
    ...
    fn startup_from_schema_28() {
        ...
        // retire its private context, not merely report the row as pending.
        {
            ...
            let archive: Receipt =
                serde_json::from_slice(&fs::read(&archives[0]).unwrap()).unwrap();
        ...
        // first announced run from schema 28 drains generation one backlog
        {
            let temporary = tempfile::tempdir().unwrap();
            let _environment = crate::session::test_support::isolate_app_dir_at(temporary.path());
            let home = dirs::home_dir().unwrap();
            ...
            assert_eq!(archives.len(), 1);
            let archive: Receipt =
                serde_json::from_slice(archives.values().next().unwrap()).unwrap();
            assert_eq!(archive.phase, Phase::Committed);
        ...
        // startup from schema 28 leaves generation one backlog untouched
        {
            let temporary = tempfile::tempdir().unwrap();
            let _environment = crate::session::test_support::isolate_app_dir_at(temporary.path());
            let home = dirs::home_dir().unwrap();
    ...
    #[test]
    #[serial_test::serial]
    fn declared_and_owned_roots_keep_their_content() {
        // shared declared root preserves each agents configuration
        {
            let temporary = tempfile::tempdir().unwrap();
            let _environment = crate::session::test_support::isolate_app_dir_at(temporary.path());
            let home = dirs::home_dir().unwrap();
            ...
            let instance =
                crate::session::Instance::new("shared", temporary.path().to_str().unwrap());
            let roots = container_config::sandbox_content_roots(
                "claude",
                None,
                &config.session,
        ...
        // shared declared root never restages certified owned content
        {
            let temporary = tempfile::tempdir().unwrap();
            let _environment = crate::session::test_support::isolate_app_dir_at(temporary.path());
            let home = dirs::home_dir().unwrap();
        ...
        // owned root adds an alias role without overwriting local state
        {
            let temporary = tempfile::tempdir().unwrap();
            let _environment = crate::session::test_support::isolate_app_dir_at(temporary.path());
            let home = dirs::home_dir().unwrap();
        ...
        // pending original cannot be bypassed by role or tool changes
        {
            let temporary = tempfile::tempdir().unwrap();
            let _environment = crate::session::test_support::isolate_app_dir_at(temporary.path());
            let home = dirs::home_dir().unwrap();
