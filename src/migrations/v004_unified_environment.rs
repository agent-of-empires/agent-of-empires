...
mod tests {
    ...
    #[test]
    fn unified_environment_cases() {
        // migrate env values into environment
        {
    ...
    "#;
        ...
        // migrate empty environment values
        {
            let dir = tempfile::TempDir::new().unwrap();
            let config_path = dir.path().join("config.toml");
    ...
    "#;
        ...
        // migrate creates environment array if missing
        {
            let dir = tempfile::TempDir::new().unwrap();
            let config_path = dir.path().join("config.toml");
