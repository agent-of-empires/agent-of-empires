...
WORKER = ROOT / ".aoe-build" / "venv" / "bin" / "aoe-kanban-lite-worker"
...
def test_worker_startup_and_refresh(worker_path: Path) -> None:
    ...
    try:
        ...
        assert "filter-facet" in slots
        assert "row-column" in slots
        ...
        _write_line(
            ...
            {
                "jsonrpc": "2.0",
                "method": "plugin.settings.changed",
                "params": {"revision": 2, "changed_keys": ["default_grouping"]},
            },
        )

        # It will re-read settings; reply to those.
