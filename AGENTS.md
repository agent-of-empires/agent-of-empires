...
- `src/process/`: OS-specific process handling (`macos.rs`, `linux.rs`) plus the protocol-agnostic worker-subprocess substrate that `src/acp/` consumes and the plugin host will reuse: `worker.rs` (process-group signalling, liveness, on-disk worker paths), `worker_registry.rs` (on-disk registry of detached workers), and `runner.rs` (the `aoe __acp-runner` shim that owns an agent subprocess and outlives `aoe serve`).
